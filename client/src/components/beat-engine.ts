...
export function getTimbukMessage(beatId: BeatId, state: ConversationState): string {
  ...
  switch (beatId) {
    ...
    case "welcome": {
      ...
      if (dayNum === 5) {
        if (isPerfect) return `${name}, five days in — and yesterday you were flawless. Today is the absolute favourite because today we stop practicing and start using this for real life. We are going to fill your palace with things you actually need to remember. A grocery list. Appointments. Things from your week. This is what the memory palace was built for. I have a small surprise waiting for you at the end.`;
        if (isGood) return `${name}, five days in and you are doing beautifully. Today is the absolute favourite — because today we make this useful. Real items. Real life. Groceries, appointments, things from your week. This is what the memory palace was always meant for. Ready to see how practical this gets? I have a small surprise waiting for you at the end.`;
        return `${name}, five days. I want you to hear this — showing up every day IS the training. Today we do something different that I think is going to surprise you. We use the palace for real life. Groceries. Appointments. Things you actually need. Sometimes the technique clicks when it gets personal. Let us find out. I have a small surprise waiting for you at the end.`;
      }
      if (dayNum === 6) {
